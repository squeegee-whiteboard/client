...
class AccountField extends React.Component {
  ...
  changeUserInfo(e) {
    ...
    if (name === 'email') { changeUser.email(localStorage.getItem('JWT'), currentVal).then((result) => { console.log(result); }); } else if (name === 'username') { changeUser.username(localStorage.getItem('JWT'), currentVal).then((result) => { console.log(result); }); }
  }
